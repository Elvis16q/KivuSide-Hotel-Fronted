...
const FindBooking = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (error.response && error.response.status === 404) {
        ...
        setError(error.message); // Assuming error.response is the actual error message
      }
    } finally {
      setTimeout(() => {
        setIsLoading(false);
  ...
  const handleBookingCancellation = async (bookingId) => {
    try {
      ...
      setIsDeleted(true);
      setSuccessMessage("Booking has been cancelled succcessfully")
      setBookingInfo(clearBookingInfo);
      setConfirmationCode('');
      setError('');
      ...
      <div className="container mt-5 d-flex flex-column justify-content-center align-items-center">
        ...
        {isLoading ? (
          ...
          <div className="col-md-6 mt-4 mb-5">
            ...
            <p>
              Check-out Date: {moment(bookingInfo.checkOutDate).subtract(1, 'month').format('MMM Do, YYYY')}
            </p>
